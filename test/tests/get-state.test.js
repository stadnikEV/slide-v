...
export default function () {
  ...
  window.describe('getState( ) - return object with current position of carousel..', () => {
    window.it(`getState( ) for new SlideV({ defaultConfig }) - expected result: { currentSlideIndex: 0, numberSlidesAfterFrame: 4, lastSlideIndex: 4 }
      `, (done) => {
      ...
      chai.assert.equal(state.currentSlideIndex, 0);
    ...
    window.it(`getState( ) for new SlideV({ slidesInFrame: 3 }) - expected result: { currentSlideIndex: 0, numberSlidesAfterFrame: 2, lastSlideIndex: 4 }
      `, (done) => {
      containerElem.innerHTML = '<div>slide 0</div> <div>slide 1</div> <div>slide 2</div> <div>slide 3</div> <div>slide 4</div>';
      mySlideV = new SlideV({
        slidesInFrame: 3,
    ...
    window.it(`getState( )'for new SlideV({ slidesInFrame: 5 }) - expected result: { currentSlideIndex: 0, numberSlidesAfterFrame: 0, lastSlideIndex: 4 }
      `, (done) => {
      containerElem.innerHTML = '<div>slide 0</div> <div>slide 1</div> <div>slide 2</div> <div>slide 3</div> <div>slide 4</div>';
      mySlideV = new SlideV({
        slidesInFrame: 5,
